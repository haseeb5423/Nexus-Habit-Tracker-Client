...
const SettingsPage: React.FC<SettingsProps> = ({ settings, habits, onUpdateSettings, onResetData, onNavigate }) => {
    ...
    const ReportModal = () => {
        ...
        return (
            <div className="fixed inset-0 z-[60] bg-black/80 backdrop-blur-sm flex items-start justify-center p-0 md:p-4 pt-10 md:pt-16 animate-fade-in overflow-y-auto">
                <div className="bg-slate-50 dark:bg-brand-dark-bg w-full h-full md:h-[85vh] md:max-w-5xl md:rounded-2xl shadow-2xl flex flex-col md:flex-row overflow-hidden animate-slide-down border border-slate-200 dark:border-white/10">
                    <div className="w-full md:w-80 bg-white dark:bg-black border-b md:border-b-0 md:border-r border-slate-200 dark:border-white/10 p-6 flex flex-col gap-6 shrink-0 overflow-y-auto">
                        ...
                        <div className="hidden md:block"><h3 className="font-bold text-xl text-brand-900 dark:text-white mb-1">Generate Report</h3><p className="text-sm text-slate-500">Customize your progress report</p></div>
                        <div className="space-y-4">
                            <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider">Date Range</label>
                            <div className="grid grid-cols-1 gap-3">
                                <div><span className="text-xs text-slate-400 mb-1 block">Start Date</span><input type="date" value={reportDateRange.start} onChange={(e) => setReportDateRange({ ...reportDateRange, start: e.target.value })} className="w-full p-2 rounded-lg bg-slate-50 dark:bg-black/40 border border-slate-200 dark:border-white/10 text-slate-800 dark:text-white text-sm" /></div>
                                <div><span className="text-xs text-slate-400 mb-1 block">End Date</span><input type="date" value={reportDateRange.end} onChange={(e) => setReportDateRange({ ...reportDateRange, end: e.target.value })} className="w-full p-2 rounded-lg bg-slate-50 dark:bg-black/40 border border-slate-200 dark:border-white/10 text-slate-800 dark:text-white text-sm" /></div>
                            </div>
                        </div>
                        <div className="mt-auto pt-4 border-t border-slate-200 dark:border-white/10 space-y-3">
                            <button onClick={handleDownloadPDF} disabled={isExporting} className="w-full py-3 rounded-xl bg-brand-600 hover:bg-brand-700 text-white font-bold shadow-lg flex items-center justify-center gap-2 transition-all active:scale-95">{isExporting ? <span className="animate-pulse">Generating PDF...</span> : <><Download size={18} /> Download PDF</>}</button>
                            <button onClick={() => setShowReportModal(false)} className="w-full py-3 rounded-xl border border-slate-200 dark:border-white/10 text-brand-600 dark:text-brand-400 hover:bg-slate-100 dark:hover:bg-white/5 font-medium">Close Preview</button>
                        </div>
                    </div>
    ...
    return (
        <div className="max-w-2xl mx-auto space-y-8 animate-fade-in pb-10 relative">
            <h2 className="text-3xl font-bold text-brand-900 dark:text-white tracking-tight">Settings</h2>

            <div className="bg-white/80 dark:bg-brand-dark-surface backdrop-blur-xl p-8 rounded-2xl shadow-lg border border-slate-200 dark:border-white/10 space-y-8">
                ...
                <div>
                    <h3 className="text-lg font-semibold text-brand-900 dark:text-white mb-4 flex items-center gap-2">
                        <Tag size={20} className="text-slate-400 dark:text-white/60" /> Manage Categories
                    </h3>
                    <div className="space-y-4">
                        <div className="flex gap-2">
                            ...
                            />
                            <button onClick={handleAddCategory} className="p-2 bg-brand-600 text-white rounded-lg hover:bg-brand-700 transition-colors">
                                <Plus size={20} />
                            </button>
                        </div>
                        <div className="max-h-48 overflow-y-auto scrollbar-thin scrollbar-thumb-brand-500/20 p-1">
                            <div className="flex flex-wrap gap-2">
                                {(settings.categories || ['Wellness', 'Fitness', 'Productivity', 'Mindset', 'Social']).map(cat => (
                                    <div key={cat} className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-brand-50 dark:bg-brand-900/20 text-brand-700 dark:text-brand-300 border border-brand-100 dark:border-brand-500/20 text-xs font-bold">
                                        {cat}
                ...
                <div>
                    <h3 className="text-lg font-semibold text-brand-900 dark:text-white mb-4 flex items-center gap-2">
                        <Cloud size={20} className="text-slate-400 dark:text-white/60" /> Account & Sync
                    </h3>
                    <div className="space-y-4">
                        {currentUser ? (
                            <div className="bg-brand-50 dark:bg-brand-900/20 p-4 rounded-xl border border-brand-100 dark:border-brand-500/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                                <div className="flex items-center gap-3">
                                    <div className="bg-white dark:bg-white/10 p-2 rounded-full">
                                        <User className="text-brand-600 dark:text-brand-300" size={24} />
                                    </div>
                                    <div>
                                        <p className="font-bold text-brand-900 dark:text-white">{currentUser.name}</p>
                                        <p className="text-xs text-brand-600 dark:text-brand-400">{currentUser.email}</p>
                                    </div>
                                </div>                                 <div className="flex gap-2 w-full sm:w-auto">
                                    <button onClick={handleSyncNow} disabled={syncStatus === 'loading'} className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2 bg-brand-600 text-white rounded-lg hover:bg-brand-700 transition-colors shadow-sm text-sm font-bold">
                            ...
                            <div className="bg-slate-50 dark:bg-black/20 p-6 rounded-xl border border-slate-200 dark:border-white/10 text-center">
                                ...
                                <button onClick={() => onNavigate('/profile')} className="px-6 py-2 bg-brand-600 text-white rounded-xl font-bold hover:bg-brand-700 transition-colors shadow-lg">Log In / Sign Up</button>
                            </div>
                        )}
                ...
                {/* Danger Zone */}
                <div>
                    <h3 className="text-lg font-semibold text-brand-900 dark:text-white mb-4 flex items-center gap-2"><AlertTriangle size={20} className="text-red-500" /> Danger Zone</h3>
            ...
            {showResetModal && (
                <div className="fixed inset-0 md:left-64 z-[100] flex items-start justify-center p-4 pt-32 bg-black/60 backdrop-blur-sm animate-fade-in">
                    <div className="bg-white dark:bg-brand-dark-surface rounded-2xl shadow-2xl w-full max-w-sm p-6 border border-slate-200 dark:border-white/10 transform animate-slide-down">
                        ...
                        <p className="text-slate-600 dark:text-brand-dark-muted mb-6">This action cannot be undone.</p>
