...
const Profile: React.FC<ProfileProps> = ({ habits, onLogout, onSync }) => {
  ...
  const badges = [
    { name: 'Sprout', icon: Zap, color: 'text-teal-500', achieved: habits.some(h => calculateStreak(h).current >= 3) },
    { name: 'Sapling', icon: Shield, color: 'text-blue-500', achieved: habits.some(h => calculateStreak(h).current >= 7) },
    { name: 'Mighty Oak', icon: Award, color: 'text-brand-600', achieved: habits.some(h => calculateStreak(h).current >= 30) },
  ];
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-10 animate-fade-in">
        ...
        {/* Header Card */}
        <div className="relative bg-white dark:bg-brand-dark-surface rounded-2xl p-6 md:p-8 shadow-xl border border-slate-200 dark:border-white/10 overflow-hidden">
            {/* Background Pattern */}
            ...
            <div className="relative z-10 flex flex-col md:flex-row gap-6 items-start md:items-center justify-between">
                <div className="flex items-center gap-6">
                    <div className="w-24 h-24 rounded-full bg-brand-600 flex items-center justify-center text-white text-3xl font-bold shadow-lg">
                    ...
                    <div className="space-y-1">
                        {isEditing ? (
                            ...
                            <h2 className="text-3xl font-bold text-brand-900 dark:text-white">{user.name}</h2>
                        )}
                        <div className="flex items-center gap-2 text-slate-500 dark:text-brand-dark-muted text-sm">
                ...
                <div className="flex gap-3 self-end md:self-auto">
                    {isEditing ? (
                        <button 
                            ...
                            disabled={isSaving}
                            className="flex items-center gap-2 px-4 py-2 bg-brand-600 text-white rounded-lg hover:bg-brand-700 transition-colors shadow-lg"
                        >
                            <Check size={16} /> Save
                        </button>
                    ) : (
            ...
            <div className="mt-8">
                ...
                {isEditing ? (
                    <textarea 
                        ...
                        className="w-full p-3 rounded-xl bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 text-slate-800 dark:text-white h-24 focus:outline-none focus:ring-2 focus:ring-brand-500/50"
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-white/80 dark:bg-brand-dark-surface p-5 rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm">
                <div className="flex items-center gap-2 text-brand-600 mb-2"><Activity size={18} /> <span className="text-xs font-bold uppercase">Consistency</span></div>
        ...
        <div className="bg-white/80 dark:bg-brand-dark-surface p-6 rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm">
            <h3 className="text-lg font-bold text-brand-900 dark:text-white mb-4 flex items-center gap-2">
