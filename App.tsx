...
const App: React.FC = () => {
      ...
      if (api.isLoggedIn()) {
        ...
        if (remoteData) {
          if (remoteData.habits && remoteData.habits.length > 0) {
            // Smart Merging Strategy: Last Write Wins based on updatedAt
            const mergedHabits = [...migratedHabits];

            remoteData.habits.forEach((remoteHabit: Habit) => {
              const localIndex = mergedHabits.findIndex(h => h.id === remoteHabit.id);
              if (localIndex === -1) {
                // New habit from server
                mergedHabits.push({
                  ...remoteHabit,
                  icon: remoteHabit.icon || 'CheckSquare'
                });
              } else {
                // Conflict resolution
  ...
  const toggleCompletion = useCallback((id: string, dateStr: string, direction?: 'add' | 'remove') => {
    setHabits(prevHabits => prevHabits.map(h => {
      ...
      return { ...h, completions: newCompletions, notes: newNotes, updatedAt: new Date().toISOString() };
    }));
  }, []);
  ...
  const updateHabitNote = useCallback((id: string, dateStr: string, noteContent: string) => {
    setHabits(prevHabits => prevHabits.map(h => {
      ...
      return { ...h, notes: newNotes, updatedAt: new Date().toISOString() };
    }));
  }, []);
